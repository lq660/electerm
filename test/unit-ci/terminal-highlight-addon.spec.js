...
test('terminal highlight addon', async (t) => {
  ...
  await t.test('adds colors to plain structured logs', () => {
    ...
    const highlighted = addon.highlightKeywords(
      '2026-07-28T21:51:46.331+08:00 INFO app costMillis=2 response={"success":true,"error":null}\n'
    )
    ...
    assert.ok(highlighted.includes(`${ESC}[32mtrue${ESC}[0m`))
    assert.ok(highlighted.includes(`${ESC}[90mnull${ESC}[0m`))
  })
